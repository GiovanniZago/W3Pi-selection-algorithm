...
def main():
    ...
    with uproot.open(DATA_PATH + "l1Nano_WTo3Pion_reco_PU200.root") as f_ur:
        ev_idx_list = []
        part_idxs_list = []
        reco_mass_list = []
                ...
                for ii in range(n_hig_pt):
                    ...
                    for jj in range(n_angsep0):
                        ...
                        for kk in range(n_angsep1):
                            ...
                            charge = np.sign(pdg_ids[hig_target_idx0]) + np.sign(pdg_ids[hig_target_idx1]) + np.sign(pdg_ids[hig_target_idx2])
                            abs_charge = np.abs(charge)

                            if abs_charge == 1:
                                mass1 = 0.13957039 if (np.abs(pdg_ids[hig_target_idx0]) > 0) else 0.1349768
                                ...
                                e1 = np.sqrt((pts[hig_target_idx0] * PT_CONV * np.cosh(etas[hig_target_idx0] * F_CONV)) ** 2 + mass1 ** 2)

                                mass2 = 0.13957039 if (np.abs(pdg_ids[hig_target_idx1]) > 0) else 0.1349768
                                ...
                                e2 = np.sqrt((pts[hig_target_idx1] * PT_CONV * np.cosh(etas[hig_target_idx1] * F_CONV)) ** 2 + mass2 ** 2)

                                mass3 = 0.13957039 if (np.abs(pdg_ids[hig_target_idx2]) > 0) else 0.1349768
                                ...
                                e3 = np.sqrt((pts[hig_target_idx2] * PT_CONV * np.cosh(etas[hig_target_idx2] * F_CONV)) ** 2 + mass3 ** 2)
                                ...
                                if (invariant_mass >= 70) and (invariant_mass <= 90):
                                    # triplet_idx = (hig_target_idx0, hig_target_idx1, hig_target_idx2)
                                    # triplet_idx = tuple(sorted(triplet_idx))
                                    triplet_idxs = [hig_target_idx0, hig_target_idx1, hig_target_idx2]
                                    triplet_idxs.sort()

                                    if tuple(triplet_idxs) not in processed_triplets:
                                        ev_idx_list.append(k)
                                        part_idxs_list.append(triplet_idxs)
                                        reco_mass_list.append(invariant_mass)
                                        processed_triplets.add(tuple(triplet_idxs))
