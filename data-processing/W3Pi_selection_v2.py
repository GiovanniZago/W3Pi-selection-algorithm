...
with h5py.File(DATA_PATH + "l1Nano_WTo3Pion_PU200.hdf5", "r") as f_in:
    with h5py.File(DATA_PATH + "l1Nano_WTo3Pion_PU200_reco_v2.hdf5", "w") as f_out:
        ...
        for ev_idx in tqdm(ev_idxs):
            ...
            for i, (pt, pdg_id) in enumerate(zip(pts, pdg_ids)):
                is_hig_pt = pt >= MIN_PT * PT_CONV
                is_pdg_id = (np.abs(pdg_id) == 211) or (np.abs(pdg_id) == 11)
                if (is_filter_idx < N_HIG) and is_hig_pt and is_pdg_id:
                    is_filter[is_filter_idx] = i + 1
                    is_filter_idx += 1

            is_filter_mask = is_filter > 0
            skip_event = (np.sum(is_filter_mask) < 3)

            is_iso_filter = np.zeros_like(is_filter)
            pts_iso_filter = np.zeros_like(is_filter)
            etas_iso_filter = np.zeros_like(is_filter)
            phis_iso_filter = np.zeros_like(is_filter)
            pdg_ids_iso_filter = np.zeros_like(is_filter)
            ...
            grp_out.create_dataset("reco_w_mass", data=invariant_mass, dtype=np.float32)
